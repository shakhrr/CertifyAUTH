...
public class Token {
    ...
    private Token(Uri uri, boolean internal) throws TokenUriInvalidException {
        try {
            ...
            String str = uri.getQueryParameter("imageBitmap");
            if (str != null && !str.isEmpty()) {
                str = str.replace("_", "/");
                str = str.replace("-", "+");
                byte[] imageBytes = Base64.decode(str, Base64.DEFAULT);
                setBitmapImage(BitmapFactory.decodeByteArray(imageBytes, 0, imageBytes.length));
            }
        } catch (Exception e) {
            Logger.error("Token(Uri uri, boolean internal)", e.getMessage());
        }
    }
    ...
    public void setBitmapImage(Bitmap bitmap) {
        ...
        imageBitmapAlt = this.imageBitmap = bitmap;
    }

    public Bitmap getImageBitmap() {
        if (this.imageBitmapAlt != null)
            return imageBitmapAlt;
        return imageBitmap;
    }
}
