...
public class TokenPersistence {
    ...
    private List<String> getTokenOrder() {
        ...
        String str;
        try {
            str = AESCrypt.decrypt(SECRET_KEY, prefs.getString(ORDER, "[]"));
        } catch (Exception e) {
            str = prefs.getString(ORDER, "[]");
        }
    ...
    private SharedPreferences.Editor setTokenOrder(List<String> order) {
        try {
            return prefs.edit().putString(ORDER, AESCrypt.encrypt(SECRET_KEY, gson.toJson(order)));
        } catch (Exception e) {
    ...
    private static class SaveTokenTask extends AsyncTask<TaskParams, Void, ReturnParams> {
        protected ReturnParams doInBackground(TaskParams... params) {
            ...
            if (taskParams.getToken().getImage() != null) {
                try {
                    ...
                    out.close();
                    taskParams.getToken().setImage(taskParams.getToken().getImage());
                } catch (IOException e) {
                    e.printStackTrace();
        ...
        protected void onPostExecute(ReturnParams returnParams) {
            ...
            returnParams.context.sendBroadcast(new Intent(UserActivity.ACTION_IMAGE_SAVED));
        }
    }
}
